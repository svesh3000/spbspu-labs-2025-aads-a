...
namespace dribas
{
  ...
  template< class Key, class T, class Compare >
  NodeType* AVLTree< Key, T, Compare >::findMin(NodeType* node) {
    if (node == fakeleaf_) {
      return fakeleaf_;
    }
    while (node->left != fakeleaf_) {
      node = node->left;
    }
    return node;
  }
  ...
  Iterator< Key, T, Compare > AVLTree< Key, T, Compare >::erase(const_iterator position)
  {
    ...
    NodeType* nodeToBalanceFrom = nullptr;
    
    // Case 1: Node to delete is a leaf
    if (nodeToDelete->left == fakeleaf_ && nodeToDelete->right == fakeleaf_) {
      nodeToBalanceFrom = nodeToDelete->parent;
      if (nodeToBalanceFrom) {
        if (nodeToBalanceFrom->left == nodeToDelete) {
          nodeToBalanceFrom->left = fakeleaf_;
        } else {
          nodeToBalanceFrom->right = fakeleaf_;
        }
        fakeleaf_->parent = nodeToBalanceFrom; // Ensure fakeleaf's parent is updated if it was pointing to nodeToDelete
      } else {
        root_ = fakeleaf_; // Tree becomes empty
        fakeleaf_->parent = fakeleaf_; // Reset fakeleaf's parent
      }
      delete nodeToDelete;
      --size_;
    }
    // Case 2: Node to delete has one child
      ...
      if (nodeToBalanceFrom) {
        if (nodeToBalanceFrom->left == nodeToDelete) {
          nodeToBalanceFrom->left = child;
        } else {
          nodeToBalanceFrom->right = child;
        }
      } else {
        root_ = child; // Child becomes the new root
      }
      child->parent = nodeToBalanceFrom;
      delete nodeToDelete;
      --size_;
    }
    // Case 3: Node to delete has two children
    else {
      NodeType* successor = findMin(nodeToDelete->right);
      
      // Swap values
      std::swap(nodeToDelete->value, successor->value);

      // successor has at most one right child (because it's the minimum in its subtree)
      nodeToBalanceFrom = successor->parent;
      if (nodeToBalanceFrom->left == successor) {
        nodeToBalanceFrom->left = successor->right;
      } else {
        nodeToBalanceFrom->right = successor->right;
      }
      if (successor->right != fakeleaf_) {
        successor->right->parent = nodeToBalanceFrom;
      }
      delete successor;
      --size_;
    }
  ...
  void AVLTree< Key, T, Cmp >::balanceTree(NodeType* node)
  {
    while (node != nullptr && node != fakeleaf_) {
      node = balance(node);
      if (node == root_) { // Если текущий узел - корень, балансировка закончена
        break;
      }
      node = node->parent;
    }
