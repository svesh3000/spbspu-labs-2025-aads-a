...
namespace
{
  void printTreeValues(std::ostream & out, bocharov::Tree< size_t, std::string > tree)
  {
...
{
  bocharov::Tree< int, std::string > tree;
...
{
  bocharov::Tree< size_t, std::string > t;
  bocharov::Tree< size_t, std::string > tree(std::move(t));
  ...
  bocharov::Tree< size_t, std::string > cpy_tree(tree);
...
{
  bocharov::Tree< size_t, std::string > tree;
  tree.insert(std::make_pair(1, "1"));
  tree.insert(std::make_pair(10, "10"));
  tree.insert(std::make_pair(3, "3"));

  bocharov::Tree< size_t, std::string > tree_cpy(tree);
  BOOST_TEST(tree_cpy.size() == 3);
  BOOST_TEST((*tree_cpy.begin()).second == "1");

  bocharov::Tree< size_t, std::string > tree_mv(std::move(tree_cpy));

  bocharov::Tree< size_t, std::string > range_tree(++tree_mv.begin(), tree_mv.end());
  std::ostringstream out;
  printTreeValues(out, range_tree);
  BOOST_TEST(out.str() == "310");

  bocharov::Tree< size_t, std::string > ilist_tree{ { 1, "1" }, { 2, "2" }, { 0, "0" } };
...
{
  bocharov::Tree< size_t, std::string > tree1;
  bocharov::Tree< size_t, std::string > tree2;
...
BOOST_AUTO_TEST_CASE(operators_test)
{
  ...
  cpy_tree.erase(2);
  bocharov::Tree< size_t, std::string > mv_tree = std::move(cpy_tree);
  std::ostringstream out2;
