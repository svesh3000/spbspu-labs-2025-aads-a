...
zholobov::CircularFwdList< T >::CircularFwdList(const CircularFwdList& other):
  head_(nullptr), tail_(nullptr), size_(0)
{
  Node_t* other_p = other.head_;
  if (other_p != nullptr) {
    head_ = new Node_t;
    head_->value = other_p->value;
    head_->next = head_;
    ++size_;
