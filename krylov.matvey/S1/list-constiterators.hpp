...
namespace krylov
{
  ...
  public:
    ...
    ConstIterator();
    ConstIterator(const Node< T >* node) noexcept;
    ConstIterator(const Node< T >* node, const List< T >* list) noexcept;
  ...
  template< typename T >
  ConstIterator< T >::ConstIterator(const Node< T >* node, const List< T >* list) noexcept:
    current_(node),
    list_(list)
  {}
